...
req-sender は対象オリジンが以下2点を満たしていることを前提とする：

1. **CORS ヘッダの返却** — `Access-Control-Allow-Origin: *`, `Access-Control-Allow-Methods: *`, `Access-Control-Allow-Headers: *` 相当。`OPTIONS` への 2xx 応答も必要
2. **CORS preflight が前段 WAF を通過すること** — カスタムヘッダや非 simple な Content-Type を使うプリセットは OPTIONS が先に飛ぶ。CDN/WAF が OPTIONS を弾くと本リクエストに到達しない

zresp は (1) を CORS ミドルウェアとしてリポジトリレベルで組み込み済み。
(2) は Cloudflare 配下なら Custom rule で対応：

```
(http.host eq "<zresp host>" and http.request.method eq "OPTIONS")
```

このルールを **Skip** アクションに設定し、Bot Fight Mode・Managed Rules・Rate Limiting 等を OPTIONS だけ対象外にする。これにより preflight だけ無検査で通り、本リクエスト (実ペイロード入り) は通常通り WAF を通過する。
...
別ホストから使いたければ `companion.py` の `ALLOWED_ORIGINS` を編集する。

### 注意: Cloudflare WARP / One Client

Companion モードで起動した curl / python / go から出ていく通信が、**Cloudflare One Client | WARP** 経由になっていると、送信元 IP が Cloudflare 自身の網内になる。Cloudflare Bot Management はこれを「Cloudflare 内部からの通信」として扱うため、`cf-bot-score` などの判定が崩れる (本来 1 になるべき curl が 99 で返ってくる、など)。

Bot 検知挙動を見たい時は、Companion 利用前に Cloudflare One Client | WARP を **OFF** にしてください。
